 ...
 #       label += 't'
  ...
  #      continue
      ...
      for higgsGrandDaughter in higgsGrandDaughters:
        ...
        if abs(higgsGrandDaughter.pdgId) in [ 11 ]:
          nof_ele += 1
        elif abs(higgsGrandDaughter.pdgId) in [ 13 ]:
          nof_mu += 1
...
for entry in sorted(counter.items(), key = lambda x: x[1], reverse = True):
  ...
  percentage=float(entry[1])/float(nof_entries)
  # print( "{} -> {} ({}%)".format(entry[0],entry[1],percentage ))
  print("{} -> {} ({:.1f}%)".format(entry[0], entry[1], percentage*100))
